...
from .CarataPlasticidad import CartaPlasticidad
...
def Clasificacion():
    ...
    def limpiayfinos():    
        if IP > 7 and Cu >= 6 and 1<= Cc <= 3 or 0.9*(Ll-8) > IP > 0.73*(Ll-20):
            ...
            print("El suelo es un SW-SM: Arena bien gradada con limo")     #Devuelve el tipo de suelo     
            CartaPlasticidad()    
        elif IP < 4 and Cu<6 and (Cc < 1 or Cc > 3) or IP < 0.73*(Ll-20):
            print("El suelo es un SP-SM: Arena mal gradada con limo ")          #Devuelve el tipo de suelo   
            CartaPlasticidad()       
        else:
